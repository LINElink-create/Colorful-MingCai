...
7. Popup 再通过消息读取当前页或全局数据，执行导出、导入、清空等操作
...
2. `GET_CURRENT_PAGE_ANNOTATIONS`
3. `CLEAR_CURRENT_PAGE_ANNOTATIONS`
4. `EXPORT_ANNOTATIONS`
...
它还没有做到非常强的“主文本 + 前后文 + 多候选 disambiguation”恢复算法。

所以如果你以后想优化恢复率，这一层就是第一优先级。
...
- “渲染成功”不等于“存储成功”
- “存储被清空”也不等于“页面上立刻移除 DOM 高亮”

这也是你现在读代码时一个很值得注意的现实边界。
...
4. `exportAnnotationBundle`
5. `importAnnotationBundle`
...
### 为什么 clearCurrentPage 现在不像彻底删除

因为当前实现主要清掉的是存储层数据。

页面里已经渲染出来的 `mark` 节点，没有建立一条“清空时同步移除 DOM”的完整消息链路。

所以刷新页面后会消失，但点击按钮时未必立刻消失。
...
2. 清空当前页为什么没有即时移除 DOM 高亮
重点读：
- [src/features/popup/usePopupState.ts](src/features/popup/usePopupState.ts)
- [src/modules/messaging/messageRouter.ts](src/modules/messaging/messageRouter.ts)
- [src/modules/annotations/rendering/highlightRenderer.ts](src/modules/annotations/rendering/highlightRenderer.ts)

这两个问题正好对应这个项目现在最明显的两个“从 MVP 走向可用产品”的改进点。